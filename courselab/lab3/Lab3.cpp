...
bool ifRotate = false;			//	茶壶是否旋转
float position[] = {0,0};		//茶壶绘制初始x,y坐标
...
void Draw_Scene()
{
	...
	glRotatef(teapotRotate,0,1,0);				//绕Y轴旋转
	glutSolidTeapot(1);
	glPopMatrix();
...
void updateView(int width, int height)
{
	...
	if (bPersp){
		...
		gluPerspective(45,whRatio,1,20);		//透视投影gluPerspective(GLdouble fovy,GLdouble aspect,GLdouble zNear,GLdouble zFar)，fovy眼睛看的角度
												//	aspect窗口从横比，zNear近处截面
	}
	else
...
void redraw()
{
	...
	glRotatef(fRotate, 0, 1.0f, 0);			// Rotate around Y axis		//整体旋转
	glRotatef(-90, 1, 0, 0);
